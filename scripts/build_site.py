...
def row_html(p,link):
    ...
    return (f'<li class="row" data-tags="{html.escape(tags,quote=True)}">'
            f'<a class="row-title" href="{link}">{html.escape(title_of(p))}</a>'
...
def shell(title,content,rel='',search=False):
    nav=''.join(f'<a href="{rel}{k}/index.html">{v}</a>' for k,v in [('hubs','Hubs'),('weekly','Weekly'),('entities','Entities'),('daily','Digests'),('summaries','Stories'),('concepts','Concepts'),('comparisons','Compare')])
...
def main():
    ...
    for folder,label in LABELS.items():
        ...
        rows=''.join(row_html(p,f'{p.stem}.html') for p in arr)
        ...
        (OUT/folder/'index.html').write_text(shell(label,body,'../'))
    ...
    body=stats+hero+'<div class="home-grid"><div>'+section_list('Latest Digests',daily,6)+section_list('Recently Updated',entities+concepts,8)+'</div><div>'+most+section_list('Explore Hubs',sorted((WIKI/'hubs').glob('*.md')),5)+'</div></div>'
