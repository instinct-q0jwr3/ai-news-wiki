...
def pager_html(page,tpages):
    if tpages<=1: return ''
    parts=[]
    for i in range(1,tpages+1):
        if i==page: parts.append(f'<b>{i}</b>')
        else:
            h='index.html' if i==1 else f'page-{i}.html'
            parts.append(f'<a href="{h}">{i}</a>')
...
def section_list(name,files,limit=5,show_kind=False,state=False):
    rows=[]
...
def main():
    ...
    for p in files:
        ...
        if p.name=='log.md':
            _l=text.split('\n'); _e=[l for l in _l if l.startswith('## ')]
            if len(_e)>50: text='\n'.join(_l[:3])+'\n'+'\n'.join(_e[-50:])+f'\n\n_Latest 50 of {len(_e)} entries._'
    ...
    for folder,label in LABELS.items():
        ...
        if folder=='summaries':
            ...
            daypages=[days[i:i+7] for i in range(0,len(days),7)]
            for pi,chunk in enumerate(daypages,1):
                parts=[]
                for i,day in enumerate(chunk):
                    ...
                    openattr=' open' if pi==1 and i<3 else ''
                ...
                pg=pager_html(pi,len(daypages))
                pinfo=f'Page {pi} of {len(daypages)} \u00b7 ' if len(daypages)>1 else ''
                body=(f'<div class="page-title"><span class="eyebrow">Library</span><h1>{label}</h1>'
                      f'<p>{pinfo}{len(arr)} pages maintained from the corpus, grouped by first-seen date.</p></div>'
                      f'{filter_bar(tagcount)}<div class="tree">' + ''.join(parts) + f'</div>{pg}{FILTER_JS_GROUPS}')
                (OUT/folder/('index.html' if pi==1 else f'page-{pi}.html')).write_text(shell(label,body,'../',section=folder))
        else:
            chunks=[arr[i:i+40] for i in range(0,len(arr),40)] or [[]]
            for pi,chunk in enumerate(chunks,1):
                rows=''.join(row_html(p,f'{p.stem}.html',is_new=False) for p in chunk)
                pg=pager_html(pi,len(chunks))
                pinfo=f'Page {pi} of {len(chunks)} \u00b7 ' if len(chunks)>1 else ''
                body=(f'<div class="page-title"><span class="eyebrow">Library</span><h1>{label}</h1>'
                      f'<p>{pinfo}{len(arr)} pages maintained from the corpus.</p></div>'
                      f'{filter_bar(tagcount)}<ul class="row-list">{rows}</ul>{pg}{FILTER_JS}')
                (OUT/folder/('index.html' if pi==1 else f'page-{pi}.html')).write_text(shell(label,body,'../',section=folder))
    ...
    wnum=re.search(r'W(\d+)',latest.stem).group(1) if latest and re.search(r'W(\d+)',latest.stem) else None
    wlink=f'<section><div class="section-head"><h2>Weekly synthesis</h2></div><p style="margin:14px 0 0"><a class="cta" href="{latest.relative_to(WIKI).with_suffix(".html").as_posix() if latest else "weekly/index.html"}">Summary for Week {wnum} \u2192</a></p></section>' if latest else ''
    intro='<div class="page-title"><span class="eyebrow">AI News Wiki</span><h1>A cumulative, cross-linked map of AI news.</h1><p>Every story has its own summary and original source.</p></div>'
    ...
    if NEW_STORIES:
        ...
        newsec=''
    body=intro+stats+newsec+wlink+'<div class="home-grid"><div>'+section_list('Daily',daily,7)+section_list('Recently Updated',entities[:4]+concepts[:4],8,show_kind=True,state=True)+'</div><div>'+most+'</div></div>'
