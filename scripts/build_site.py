...
def section_cards(name,files,limit=5):
    ...
    for p in files[:limit]:
        rel=p.relative_to(WIKI).with_suffix('.html').as_posix(); cards.append(f'<li><a href="{rel}">{html.escape(title_of(p))}</a></li>')
